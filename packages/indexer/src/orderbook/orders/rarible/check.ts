    ...
    if (
      ...
      if (!nftApproval) {
        if (options?.onChainApprovalRecheck) {
          ...
          const contract =
            kind === "erc721"
              ? new Sdk.Common.Helpers.Erc721(getBaseProvider(), order.params.make.assetType.contract!)
              : new Sdk.Common.Helpers.Erc1155(getBaseProvider(), order.params.make.assetType.contract!);
